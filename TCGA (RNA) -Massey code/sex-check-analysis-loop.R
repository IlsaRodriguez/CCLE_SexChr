...
study <- 'COAD'
# Set directories
setwd('C:/Users/livin/ASU FILES/Reseach Files/Genomic Sex Chromosome/TCGA_RNAseq_counts') #make sure you change this to reflect your file path
...
#  I've updated the code to incorporate an if-else statement. This modification ensures that if there are no samples meeting the criteria of 'yes XIST and yes Y,' instead of encountering an error, the output will be a message indicating the absence of such samples, such as 'No samples found for yes XIST and yes Y.' This enhancement aims to handle cases where there might be a lack of data conforming to the specified conditions and provides a more informative response in such scenarios. - Ilsa 
